...
namespace InmobiliariaAlbornoz.Api
{
    ...
    public class PropietariosController : ControllerBase
    {
        ...
        /*[HttpPost]
        [AllowAnonymous]
        public async Task<IActionResult> CambiarClaveConToken2([FromForm] string token, [FromForm] string nuevaClave)
        {
            try
            {
                // Validar el token JWT
                var tokenHandler = new JwtSecurityTokenHandler();
                var key = System.Text.Encoding.ASCII.GetBytes(config["TokenAuthentication:SecretKey"]);
                var tokenValidationParameters = new TokenValidationParameters
                {
                    ...
                    ClockSkew = TimeSpan.Zero // No se permite retraso para la validación
                };
                ...
                // Buscar al propietario con el email extraído
                var propietario = await contexto.Propietario.FirstOrDefaultAsync(p => p.Email == emailClaim);
                if (propietario == null)
                {
                    return NotFound("No se encontró un propietario con el email asociado al token.");
                }
